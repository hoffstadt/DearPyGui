...
with window("Primary Window Tester", show=True, no_resize=True):
    ...
    add_button(
        "This Primary",
        callback=lambda sender, data: set_primary_window("Primary Window Tester", True),
    )
    add_button(
        "Demo Primary",
        callback=lambda sender, data: set_primary_window("Dear PyGui Demo", True),
    )
...
with window("Asyncronous##dialog", show=True):
    ...
    def LongAsyncronousCallback(sender, data):
        ...
        run_async_function(
            LongCallback, current_number, return_handler=ReturnFromLongProcess
        )
    ...
    def LongAsyncronousCallback2(sender, data):
        current_number = get_data("threadNumber")
        run_async_function(
            LongCallback2, current_number, return_handler=ReturnFromLongProcess
        )
        log_info("Start process number: " + str(current_number))
        add_data("threadNumber", current_number + 1)
    ...
    add_button(
        "Start Long Process Lambda",
        callback=lambda sender, data: run_async_function(
            lambda sender, data: time.sleep(5),
            None,
            return_handler=lambda sender1, data2: print("done!"),
        ),
    )
