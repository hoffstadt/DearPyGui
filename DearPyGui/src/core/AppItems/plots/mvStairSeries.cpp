...
namespace Marvel {
	...
	mvStairSeries::mvStairSeries(const std::string& name, const std::vector<std::vector<float>>& default_value)
	...
	PyObject* mvStairSeries::add_stair_series(PyObject* self, PyObject* args, PyObject* kwargs)
	{
		static int i = 0; i++;
		...
		auto item = CreateRef<mvStairSeries>(name, std::vector<std::vector<float>>{xs, ys});
