...
namespace Marvel {
	...
	mvAreaSeries::mvAreaSeries(const std::string& name, const std::vector<std::vector<float>>& default_value)
	...
	PyObject* mvAreaSeries::add_area_series(PyObject* self, PyObject* args, PyObject* kwargs)
	{
		static int i = 0; i++;
		...
		if (!(mvApp::GetApp()->getParsers())[s_command].parse(args, kwargs, __FUNCTION__,
			&name, &x, &y, &fill, &label, &source, &parent, &before, &show, &axis, &contribute_to_bounds))
			return GetPyNone();

		auto xs = ToFloatVect(x);
		auto ys = ToFloatVect(y);

		auto item = CreateRef<mvAreaSeries>(name, std::vector<std::vector<float>>{xs, ys});
