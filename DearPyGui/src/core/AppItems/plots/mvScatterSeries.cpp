...
namespace Marvel {
	...
	mvScatterSeries::mvScatterSeries(const std::string& name, const std::vector<std::vector<float>>& default_value)
	...
	PyObject* mvScatterSeries::add_scatter_series(PyObject* self, PyObject* args, PyObject* kwargs)
	{
		static int i = 0; i++;
		...
		auto item = CreateRef<mvScatterSeries>(name, std::vector<std::vector<float>>{xs, ys});
