...
namespace Marvel {
	...
	void mvScatterSeries::InsertParser(std::map<std::string, mvPythonParser>* parsers)
	{
		...
		mvPythonParser parser(mvPyDataType::UUID, "Adds a scatter series to a plot.", { "Plotting", "Containers", "Widgets" });
	...
	void mvScatterSeries::draw(ImDrawList* drawlist, float x, float y)
	{
		...
		//-----------------------------------------------------------------------------
		{
			...
			ImPlot::PlotScatter(_label.c_str(), xptr->data(), yptr->data(), (int)xptr->size());
