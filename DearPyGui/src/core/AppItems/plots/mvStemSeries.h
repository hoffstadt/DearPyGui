...
namespace Marvel {
	...
	MV_REGISTER_WIDGET(mvStemSeries, MV_ITEM_DESC_DEFAULT, StorageValueTypes::VectFloatVect, 1);
	...
	public:
		...
		MV_APPITEM_TYPE(mvAppItemType::mvStemSeries, add_stem_series)
	...
	public:
		...
		mvStemSeries(const std::string& name, const std::vector<std::vector<float>>& default_value);
