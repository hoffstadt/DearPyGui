...
namespace Marvel {
	...
	void mvWindowAppItem::InsertParser(std::map<std::string, mvPythonParser>* parsers)
	{
		...
		parser.addArg<mvPyDataType::IntList>("min_size", mvArgType::KEYWORD, "[32, 32]", "Minimum window size.");
		parser.addArg<mvPyDataType::IntList>("max_size", mvArgType::KEYWORD, "[30000, 30000]", "Maximum window size.");

		parser.addArg<mvPyDataType::Bool>("menubar", mvArgType::KEYWORD, "False");
		parser.addArg<mvPyDataType::Bool>("collapsed", mvArgType::KEYWORD, "False", "Collapse the window");
		...
		parser.addArg<mvPyDataType::Callable>("on_close", mvArgType::KEYWORD, "None", "Callback ran when window is closed");
