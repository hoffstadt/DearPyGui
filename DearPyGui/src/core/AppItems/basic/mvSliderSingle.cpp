...
namespace Marvel {
    ...
    void mvSliderFloat::InsertParser(std::map<std::string, mvPythonParser>* parsers)
    {
        ...
        args.push_back({ mvPyDataType::Bool, "vertical", mvArgType::KEYWORD_ARG, "False", "Sets orientation to vertical." });
        ...
        //-----------------------------------------------------------------------------
        {
            ...
            if (_vertical)
            {
                ...
                if (ImGui::VSliderInt(_internalLabel.c_str(), ImVec2((float)_width, (float)_height), _enabled ? _value.get() : &_disabled_value, _min, _max, _format.c_str()))
                {
                    ...
                    if(_alias.empty())
                        GContext->callbackRegistry->submitCallback([=]() {
                            GContext->callbackRegistry->addCallback(getCallback(false), _uuid, ToPyInt(value), _user_data);
                            });
                    else
                        GContext->callbackRegistry->submitCallback([=]() {
                            GContext->callbackRegistry->addCallback(getCallback(false), _alias, ToPyInt(value), _user_data);
                            });
                }
            }
            else
