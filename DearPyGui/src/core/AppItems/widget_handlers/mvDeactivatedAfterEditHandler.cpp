...
namespace Marvel {
	...
	mvDeactivatedAfterEditHandler::mvDeactivatedAfterEditHandler(const std::string& name)
		:
		mvAppItem(name)
	{
	...
	void mvDeactivatedAfterEditHandler::draw(ImDrawList* drawlist, float x, float y)
	{
		...
		if (ImGui::IsItemDeactivatedAfterEdit())
		{
			mvApp::GetApp()->getCallbackRegistry().submitCallback([=]()
				{
					mvApp::GetApp()->getCallbackRegistry().runCallback(getCallback(false), m_name, GetPyNone());
				});
		}
	}
