...
def show_demo():
    ...
    with dpg.window():
        ...
        with dpg.menu_bar() as menu_bar:
            ...
            with dpg.menu(label="menu", payload_type="str", drop_callback=testy, drag_callback=testy):
                dpg.add_menu_item(label="menu item")

            with dpg.drag_payload(parent=dpg.last_container(), drop_data="dropped", drag_data="dragged", user_data="user data", payload_type="str"):
                dpg.add_text(dpg.get_item_type(dpg.last_container()))
                dpg.add_text(f"Item ID: {dpg.last_container()}")

            with dpg.drag_payload(parent=menu_bar, drop_data="dropped", drag_data="dragged", user_data="user data", payload_type="str"):
                dpg.add_text(dpg.get_item_type(menu_bar))
                dpg.add_text(f"Item ID: {menu_bar}")


        # testing basic
        with dpg.collapsing_header(label="basic") as basic:
            dpg.add_image("__demo_static_texture_1")
        ...
        items = dpg.get_item_children(basic, slot=1) 
        items += dpg.get_item_children(color, slot=1) 
        items += dpg.get_item_children(containers, slot=1)
        items += dpg.get_item_children(custom, slot=1)
        #items += dpg.get_item_children(drawlist, slot=1)
