...
- [ ] V 1.85 (https://github.com/ocornut/imgui/releases/tag/v1.85)
    - [ ] Removed GetWindowContentRegionWidth() function. (Maybe nothing to do)
    - [ ] Debug: New StackTool.
...
- [ ] V 1.86 (https://github.com/ocornut/imgui/releases/tag/v1.86)
    - [ ] Removed CalcListClipping() function. Prefer using ImGuiListClipper which can return non-contiguous ranges. (maybe nothing to do)
    - [ ] Backends: OSX: Added NSView* parameter to ImGui_ImplOSX_Init() (we don't/can't really do much about this)
    - [ ] Added GetMouseClickedCount() function, returning the number of successive clicks (so IsMouseDoubleClicked(ImGuiMouseButton_Left) is same as GetMouseClickedCount(ImGuiMouseButton_Left) == 2, but it allows testing for triple clicks and more).
...
- [ ] V 1.89.3 (https://github.com/ocornut/imgui/releases/tag/v1.89.3)
    - [ ] Backends+Examples: SDL2: renamed all unnumbered references to "sdl" to "sdl2". This is in prevision for the future release of SDL3 and its associated backend.
    - [ ] SeparatorText(): Added SeparatorText() widget.
...
- [ ] V 1.89.6 (https://github.com/ocornut/imgui/releases/tag/v1.89.6)
    - [ ] Clipper: Commented out obsolete redirection constructor which was marked obsolete in 1.79: ImGuiListClipper(int items_count, float items_height) --> Use ImGuiListClipper() + clipper.Begin().
    - [ ] Clipper: Renamed ForceDisplayRangeByIndices() to IncludeRangeByIndices()
    - [ ] Check other little breaking changes in link
